import ast
import importlib
import os
import subprocess
from typing import Optional

import typer
from picsellia.exceptions import ResourceConflictError
from picsellia.types.enums import InferenceType, Framework

from picsellia_cli.utils.session_manager import session_manager
from picsellia import Client

app = typer.Typer(
    help="Deploy pipelines: build, push Docker image, and register on Picsellia."
)


def build_and_push_docker_image(pipeline_name: str, image_name: str, image_tag: str):
    """
    Build and push the Docker image for a given pipeline, excluding virtual environments.
    """
    full_image_name = f"{image_name}:{image_tag}"
    repo_root = os.getcwd()
    pipeline_dir = os.path.join(repo_root, pipeline_name)

    if not os.path.exists(pipeline_dir):
        typer.echo(f"⚠️ Pipeline directory '{pipeline_dir}' not found.")
        raise typer.Exit()

    dockerfile_path = os.path.join(pipeline_dir, "Dockerfile")
    dockerignore_path = os.path.join(pipeline_dir, ".dockerignore")

    if not os.path.exists(dockerfile_path):
        typer.echo(f"⚠️ Missing Dockerfile in '{pipeline_dir}'.")
        raise typer.Exit()

    # Ensure .dockerignore exists to avoid including unnecessary files
    if not os.path.exists(dockerignore_path):
        with open(dockerignore_path, "w") as f:
            f.write(".venv/\nvenv/\n__pycache__/\n*.pyc\n*.pyo\n.DS_Store\n")

    # Build Docker Image
    typer.echo(
        f"🚀 Building Docker image '{full_image_name}', excluding virtual environments..."
    )
    subprocess.run(
        ["docker", "build", "-t", full_image_name, "-f", dockerfile_path, "."],
        cwd=repo_root,
        check=True,
    )

    # Push Docker Image
    typer.echo(f"📤 Pushing Docker image '{full_image_name}'...")
    subprocess.run(["docker", "push", full_image_name], check=True)

    typer.echo(f"✅ Docker image '{full_image_name}' pushed successfully!")


def find_hyperparameters_class(filepath: str) -> str:
    """
    Scans the file to find the first class that inherits from HyperParameters.
    Returns its full dotted import path (e.g. utils.hyperparameters.MyClass).
    """
    with open(filepath, "r") as f:
        tree = ast.parse(f.read(), filename=filepath)

    for node in tree.body:
        if isinstance(node, ast.ClassDef):
            for base in node.bases:
                if getattr(base, "id", "") == "HyperParameters":
                    return f"utils.hyperparameters.{node.name}"

    raise ValueError("No class inheriting from HyperParameters found.")


def resolve_class_from_string(path: str):
    module_path, class_name = path.rsplit(".", 1)
    module = importlib.import_module(module_path)
    return getattr(module, class_name)


def register_training_pipeline_on_picsellia(pipeline_name: str, pipeline_data: dict):
    """
    Register a training pipeline on Picsellia by creating a model and model version.

    Args:
        pipeline_name (str): Name of the pipeline = name of the model to create.
        pipeline_data (dict): Contains image_name, image_tag, parameters, etc.
    """
    global_data: Optional[dict] = session_manager.get_global_session()
    if not global_data:
        typer.echo("❌ Global session not initialized. Run `pipeline-cli init` first.")
        raise typer.Exit()

    try:
        client = Client(
            api_token=global_data["api_token"],
            organization_id=global_data["organization_id"],
        )

        # 1. Create or get model
        try:
            model = client.create_model(name=pipeline_name)
            typer.echo(f"✅ Model '{pipeline_name}' created.")
        except ResourceConflictError:
            model = client.get_model(name=pipeline_name)
            typer.echo(f"ℹ️ Model '{pipeline_name}' already exists. Reusing.")

        # 2. Ask for optional metadata
        # 2. Prompt for metadata using Enum choices
        framework_options = [f.name for f in Framework if f != Framework.NOT_CONFIGURED]
        inference_options = [
            i.name for i in InferenceType if i != InferenceType.NOT_CONFIGURED
        ]

        framework_input = typer.prompt(
            f"🧠 Select framework ({', '.join(framework_options)})", default="PYTORCH"
        )
        inference_type_input = typer.prompt(
            f"🧩 Select inference type ({', '.join(inference_options)})",
            default="OBJECT_DETECTION",
        )

        # 3. Build base_parameters dynamically from hyperparameters.py
        try:
            hyper_path = os.path.join(
                os.getcwd(), pipeline_name, "utils", "hyperparameters.py"
            )
            class_path = find_hyperparameters_class(hyper_path)
            typer.echo(f"🔍 Found hyperparameter class: {class_path}")

            HyperParamsCls = resolve_class_from_string(class_path)
            hparams_instance = HyperParamsCls(log_data={})  # Dummy log_data
            parameters = hparams_instance.to_dict()
            typer.echo("✅ Hyperparameters loaded and converted to dict.")
        except Exception as e:
            typer.echo(f"⚠️ Could not load hyperparameters class automatically: {e}")
            parameters = pipeline_data.get("parameters", {})

        print(f"Parameters: {parameters}")

        # 4. Create model version
        _ = model.create_version(
            name=pipeline_data["image_tag"],
            docker_image_name=pipeline_data["image_name"],
            docker_tag=pipeline_data["image_tag"],
            base_parameters=parameters,
            docker_flags=None,
            docker_env_variables=None,
            notebook_link=None,
            framework=Framework(framework_input),
            type=InferenceType(inference_type_input),
            description=None,
        )

    except Exception as e:
        typer.echo(f"❌ Error registering training pipeline: {e}")
        raise typer.Exit()


@app.command()
def deploy_pipeline(
    pipeline_name: str = typer.Argument(..., help="Name of the pipeline to deploy"),
):
    """
    🚀 Deploy a pipeline: build & push its Docker image, then register it on Picsellia.

    This command ensures the pipeline exists, prompts for missing Docker image details,
    builds and pushes the image, and finally registers the pipeline on Picsellia.
    """
    session_manager.ensure_session_initialized()

    # Fetch pipeline details from TinyDB
    pipeline_data = session_manager.get_pipeline(pipeline_name)
    if not pipeline_data:
        typer.echo(
            f"❌ Pipeline '{pipeline_name}' not found. Run `pipeline-cli list` to check available pipelines."
        )
        raise typer.Exit()

    # Prompt for missing Docker image details
    if not pipeline_data.get("image_name") or not pipeline_data.get("image_tag"):
        pipeline_data["image_name"] = typer.prompt("📦 Enter Docker image name")
        pipeline_data["image_tag"] = typer.prompt(
            "🏷️ Enter Docker image tag", default="latest"
        )
        session_manager.add_pipeline(pipeline_name, pipeline_data)  # Save in TinyDB

    # Build & Push Docker Image
    build_and_push_docker_image(
        pipeline_name, pipeline_data["image_name"], pipeline_data["image_tag"]
    )

    # Register the pipeline on Picsellia
    register_training_pipeline_on_picsellia(pipeline_name, pipeline_data)


if __name__ == "__main__":
    app()
